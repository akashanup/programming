...
dp[i][1] = max(dp[i-1][1], dp[i-2][0]-prices[i-1]-fee)
Since a is 1 which means the action taken on ith day is either Rest* or Buy. Therefore the max profit would be the max of Rest action(dp[i-1][1]) and Buy action(dp[i-1][0]-prices[i-1]-fee) because whenever we buy, we pay the fee also.
