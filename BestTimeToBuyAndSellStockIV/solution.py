...
Since a is 1 which means the action taken on ith day is either Rest* or Buy. Therefore the max profit would be the max of Rest action(dp[i-1][x][1]) and Buy action(dp[i-1][k-1][0]-prices[i-1]).
At last, we have to return dp[-1][k][0] because if we don't hold any stock at the end then only we can have the maximum profit.
"""
