...
Logic:
    ...
    If we deep dive the above statement, we can figure out that the array could be partitioned into two parts where the
    lengths of two parts differ by at max 1.
    ...
    Now the median would be,
        1. If l and r of the same length then, median = max(l)+min(r) / 2
    ...
    Then the length of merged array would be m+n, and it can be partitioned into two at (m+n / 2)th index or the index
    at half of the length of merged array.
    Now the question arises as to how to partition the assumed merged sorted array?
    Since we know the partition index(half), we can calculate how many values will be on the left part and right part
    of array.
    For example,
        arr1 => [1,4,5,8,10], m = 5
        arr2 => [2,3,9], n = 3
        ...
        Now the array could be partitioned into two parts of length 4 (4 elements will be in left part and 4 will be in
        right part).
    Now the question arises, which of the 4 elements would lie in the left part and which 4 will lie in the right.
    ...
    Now l1 can be determined by iterating the first array for the first 4 elements and for each element,
    we can check the corresponding values of l2 as l1+l2 == 4.
    For example, if,
    ...
    The above is a demonstration of linear search.
    Since our both arrays are sorted, we could use Binary search for an optimal solution.
