    ...
    def search(self, nums: List[int], target: int) -> bool:
        ...
        while start <= end:
            ...
            """
                ...
                2nd part: 8
                3rd part: 1-3
                Note: If nums[mid] == nums[start] then we can just skip the start value as we are just removing 
                a duplicate value.
            """
            if nums[mid] == target:
                return True
                ...
                # 1st part of array i.e., if target lies in first part of array then search only in that part
                # Else search in the remaining parts
                if nums[start] <= target < nums[mid]:
                    end = mid - 1
                else:
                    start = mid + 1
            else:
                # 3rd part of array i.e., if target lies in 3rd part then search only in that part
                # Else search in the remaining parts.
